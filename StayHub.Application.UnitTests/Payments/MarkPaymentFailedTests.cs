...
public class MarkPaymentFailedTests
{
    ...
    public async Task Handle_Should_ReturnFailure_WhenPaymentNotFound()
    {
        ...
        // Act
        var result = await _handler.Handle(new MarkPaymentFailedCommand(reference), default);
    ...
    [Fact]
    public async Task Handle_Should_MarkAsFailedAndSaveChanges_WhenPending()
    {
        // Arrange
        ...
        // Act
        var result = await _handler.Handle(new MarkPaymentFailedCommand(payment.ProviderReference), default);
